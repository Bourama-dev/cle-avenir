...
const UnifiedSignupStep1 = ({ formData, handleFieldChange, errors, onNext }) => {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Supabase redirect prend le relais
    } catch (error) {
      setGoogleLoading(false);
      toast({
        variant: 'destructive',
        title: 'Erreur Google',
        description: error.message || "Impossible de continuer avec Google."
      });
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      >
        {googleLoading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <GoogleIcon />}
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-white px-3 text-slate-400">ou avec un email</span>
        </div>
      </div>
