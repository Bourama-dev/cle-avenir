...
5. ~~Retirer `establishment_staff` et la RPC `verify_establishment_credentials`~~ — fait (2026-09-01, migration `20260901180000_drop_legacy_establishment_staff.sql`) : 0 ligne, aucune référence restante, aucune autre table dépendante. Toujours en attente : `establishment_password_history`, `establishment_code_history` (0 ligne chacune, liées à l'ancien schéma) et `educational_institutions.activation_password`/`establishment_code` (système d'activation séparé, jamais vérifié par aucun flux — `getEstablishmentByCode`/`verifyPassword` ne sont appelés nulle part).
